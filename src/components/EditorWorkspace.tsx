...
const DUCKED_SEGMENT_GAIN_DB = -12;
...
export default function EditorWorkspace() {
      ...
      return currentSegments.map((segment) =>
        ["pause", "breath", "noise"].includes(segment.reason)
          ? {
              ...segment,
              speed: 1,
              volumeGainDb: DUCKED_SEGMENT_GAIN_DB
            }
          : {
              ...segment,
              speed: playbackRate,
  ...
  const handleGenerateTranscript = useCallback(
    async (targetVideo = uploadedVideo) => {
      if (!targetVideo) {
        return;
      }
      ...
      try {
        ...
        formData.append("file", targetVideo.sourceFile);
        const response = await fetch("/api/transcribe", {
          method: "POST",
          body: formData
        });
    ...
    [playbackRate, uploadedVideo, volumeGainDb]
  );

  useEffect(() => {
    if (!uploadedVideo?.sourceFile) {
      autoGeneratedFileRef.current = null;
      return;
    }

    if (autoGeneratedFileRef.current === uploadedVideo.sourceFile) {
      return;
    }

    autoGeneratedFileRef.current = uploadedVideo.sourceFile;
    void handleGenerateTranscript(uploadedVideo);
  }, [handleGenerateTranscript, uploadedVideo]);
